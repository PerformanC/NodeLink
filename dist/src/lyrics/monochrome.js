 ...
 * @public
 */
export default class MonochromeLyrics {
    ...
    getBestInstance() {
        const now = Date.now();
        const candidates = this.instances.filter((i) => i.score > 0 || now - i.lastFailure > 30_000);
    ...
    async getLyrics(trackInfo) {
        ...
        try {
            ...
            logger('warn', 'Lyrics', `Monochrome lyrics request failed: ${e instanceof Error ? e.message : String(e)}`);
            return { loadType: 'empty', data: {} };
        }
    }
    /**
