...
export default class GeniusLyrics {
    ...
    buildSearchQuery(trackInfo) {
        ...
        const query = author.length > 0 && !title.toLowerCase().startsWith(author.toLowerCase())
            ? `${title} ${author}`
            : title;
     ...
     */
    extractSearchHitPath(body) {
        const payload = typeof body === 'string' ? JSON.parse(body) : body;
        const rootRecord = getRecordFromValue(payload ?? undefined);
        const responseRecord = getRecordFromValue(getRecordValue(rootRecord, 'response'));
        ...
        for (const sectionValue of sections) {
            ...
            const resultRecord = getRecordFromValue(getRecordValue(firstHit, 'result'));
    ...
    extractPreloadedState(html) {
        ...
        try {
            const parseFunction = new Function(`return JSON.parse(${jsonParseArgument})`);
    ...
    toPreloadedState(value) {
        ...
        const songPageRecord = getRecordFromValue(getRecordValue(rootRecord, 'songPage'));
        const lyricsDataRecord = getRecordFromValue(getRecordValue(songPageRecord, 'lyricsData'));
        const bodyRecord = getRecordFromValue(getRecordValue(lyricsDataRecord, 'body'));
    ...
    async getLyrics(trackInfo) {
        ...
        try {
            const searchResponse = await makeRequest(`https://genius.com/api/search/multi?q=${encodeURIComponent(query)}`, {
                method: 'GET',
            ...
            if (searchResponse.statusCode !== 200) {
                throw new Error(`Unexpected Genius search status code: ${searchResponse.statusCode}`);
            }
            const songPath = this.extractSearchHitPath(searchResponse.body);
            if (!songPath) {
                return { loadType: 'empty', data: {} };
            }
            const songPageResponse = await makeRequest(`https://genius.com${songPath}`, {
                method: 'GET',
                headers: { ...GENIUS_HEADERS }
            });
            ...
            if (songPageResponse.statusCode !== 200) {
                throw new Error(`Unexpected Genius page status code: ${songPageResponse.statusCode}`);
            }
            const songPageHtml = typeof songPageResponse.body === 'string' ? songPageResponse.body : '';
            ...
            const message = getErrorMessage(error instanceof Error ? error : String(error));
            logger('error', 'Lyrics', `Failed to fetch lyrics from Genius: ${message}`);
            return {
                loadType: 'error',
                data: {
                    message,
