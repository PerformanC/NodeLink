 ...
 */
function getLyricsSubscriptionRuntime(nodelink) {
 ...
 */
function getSkipTrackSource(parsedUrl) {
...
async function handler(nodelink, req, res, _sendResponse, parsedUrl) {
    ...
    if (!runtime) {
        sendErrorResponse(req, res, 500, 'Internal Server Error', 'Lyrics subscription runtime contract is incomplete.', parsedUrl.pathname, true);
        return;
    }
    ...
    if (!session.players) {
        sendErrorResponse(req, res, 500, 'Player manager not initialized', 'Player manager not initialized', parsedUrl.pathname);
        return;
    }
    if (req.method === 'POST') {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Error subscribing to lyrics';
            logger('error', 'LyricsAPI', `Error subscribing to lyrics: ${errorMessage}`);
    ...
    if (req.method === 'DELETE') {
        try {
            ...
            logger('error', 'LyricsAPI', `Error unsubscribing from lyrics: ${errorMessage}`);
            sendErrorResponse(req, res, 500, errorMessage, errorMessage, parsedUrl.pathname, true);
        }
        return;
    }
