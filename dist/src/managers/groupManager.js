...
export default class GroupManager {
    ...
    create(id, guildIds = []) {
        ...
        logger('debug', 'GroupManager', `Group '${id}' created with ${guildIds.length} player(s)`);
        return group;
    }
    /**
     ...
     */
    addPlayer(groupId, guildId) {
        const group = this.groups.get(groupId);
        if (!group)
            throw new Error(`Group '${groupId}' not found`);
        group.guildIds.add(guildId);
        logger('debug', 'GroupManager', `Added guild ${guildId} to group '${groupId}' (${group.guildIds.size} member(s))`);
    }
    /**
     ...
     * @throws Error if the group does not exist.
     */
    removePlayer(groupId, guildId) {
        const group = this.groups.get(groupId);
        if (!group)
            throw new Error(`Group '${groupId}' not found`);
        group.guildIds.delete(guildId);
        logger('debug', 'GroupManager', `Removed guild ${guildId} from group '${groupId}' (${group.guildIds.size} member(s))`);
    }
    /**
