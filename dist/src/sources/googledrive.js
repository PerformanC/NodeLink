 ...
 * @public
 */
export default class GoogleDriveSource {
    nodelink;
    searchTerms = ['gdsearch'];
    patterns = [
    ...
    priority = 90;
    apiKey = 'AIzaSyDVQw45DwoYh632gvsP5vPDqEKvb-Ywnb8';
    ...
    _makeSidAuthHeader(cookiesStr, origin) {
    ...
    async _getCookiesAndAuthHeader(url, origin) {
        ...
        const driveConfig = this.nodelink.options.sources?.googledrive;
    ...
    _extractDriveConfirmToken(html) {
    ...
    _normalizeDriveTitle(title) {
        const normalized = title.replace(' - Google Drive', '').trim();
        if (!normalized || /virus scan warning/i.test(normalized))
            return '';
        return normalized;
    }
    ...
    async _resolveFolderViaEmbedded(folderId) {
        ...
        const folderTitle = titleMatch?.[1]
            ...
            : 'Google Drive Folder';
        const entryRegex = /<div class="flip-entry" id="entry-([A-Za-z0-9_-]{20,})"[\s\S]*?<a href="https:\/\/drive\.google\.com\/file\/d\/([A-Za-z0-9_-]{20,})\/view[^"]*"[\s\S]*?drive-thirdparty\.googleusercontent\.com\/(?:128|16)\/type\/([^"/]+\/[^"/]+)[\s\S]*?<div class="flip-entry-title">([\s\S]*?)<\/div>/g;
        const tracks = [];
        const seen = new Set();
    ...
    async resolve(url) {
        ...
        const idMatch = (filePattern1 ? url.match(filePattern1) : null) ||
    ...
    async _getFileInfo(fileId, url) {
        const origin = 'https://drive.google.com';
        const { cookieHeader, authHeader } = await this._getCookiesAndAuthHeader(url, origin);
        const headers = {
        ...
        try {
            ...
            const probe = await http1makeRequest(`https://drive.google.com/uc?id=${fileId}&export=download&authuser=0`, {
                method: 'GET',
            ...
            const disposition = (Array.isArray(dispositionHeader)
                ? dispositionHeader[0]
                : dispositionHeader) || '';
            const filenameFromDisposition = this._extractFilenameFromContentDisposition(String(disposition));
        ...
        if (title === 'Unknown Drive File') {
            const downloadPage = await makeRequest(`https://drive.google.com/uc?id=${fileId}&export=download`, {
                method: 'GET',
    ...
    async _resolveFolder(folderId, url) {
        try {
            ...
            if (!keyMatch)
                throw new Error('API Key not found in folder page');
            const key = keyMatch[1];
            const boundary = '=====vc17a3rwnndj=====';
            const fields = 'kind%2CnextPageToken%2Citems(kind%2CmodifiedDate%2CmodifiedByMeDate%2ClastViewedByMeDate%2CfileSize%2Cowners(kind%2CpermissionId%2Cid)%2ClastModifyingUser(kind%2CpermissionId%2Cid)%2ChasThumbnail%2CthumbnailVersion%2Ctitle%2Cid%2CresourceKey%2Cshared%2CsharedWithMeDate%2CuserPermission(role)%2CexplicitlyTrashed%2CmimeType%2CquotaBytesUsed%2Ccopyable%2CfileExtension%2CsharingUser(kind%2CpermissionId%2Cid)%2Cspaces%2Cversion%2CteamDriveId%2ChasAugmentedPermissions%2CcreatedDate%2CtrashingUser(kind%2CpermissionId%2Cid)%2CtrashedDate%2Cparents(id)%2CshortcutDetails(targetId%2CtargetMimeType%2CtargetLookupStatus)%2Ccapabilities(canCopy%2CcanDownload%2CcanEdit%2CcanAddChildren%2CcanDelete%2CcanRemoveChildren%2CcanShare%2CcanTrash%2CcanRename%2CcanReadTeamDrive%2CcanMoveTeamDriveItem)%2Clabels(starred%2Ctrashed%2Crestricted%2Cviewed))%2CincompleteSearch';
            let pageToken = '';
            const tracks = [];
            ...
            while (pageToken !== null) {
                ...
                const ctParam = encodeURIComponent(`multipart/mixed; boundary="${boundary}"`);
                ...
                if (!jsonMatch) {
                    if (batchResponse.body.includes('400 Bad Request')) {
                        throw new Error('Google Drive API rejected batch request (400 Bad Request). Are cookies properly configured?');
                    }
            ...
            return {
                loadType: 'error',
                exception: {
                    message: `Folder resolution failed: ${err.message}`,
                    severity: 'common'
                }
            };
        }
    ...
    async loadStream(_track, url, _protocol, additionalData) {
        ...
        let streamType = contentType || this._getFormat(contentType, _track.title) || 'mp3';
        if (contentType.includes('text/html')) {
            ...
            if (typeof htmlResponse.body === 'string') {
                ...
                if (confirmToken) {
                    ...
                    const confirmedType = request.headers?.['content-type'] || '';
                ...
                return {
                    exception: {
                        message: 'Failed to bypass Drive confirmation (invalid HTML response)',
                        severity: 'common'
                    }
                };
            }
        ...
        const onError = (err) => {
            ...
            const isBenignAbort = netErr.code === 'ECONNRESET' || /aborted|socket hang up/i.test(message);
            if (isBenignAbort) {
