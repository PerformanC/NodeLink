...
export default class TV_DOWN extends BaseClient {
    ...
    getClient(context) {
        return {
            client: {
                ...
                browserName: 'Cobalt',
                userAgent: 'Mozilla/5.0 (Unknown) Cobalt/5.150.00-qa (unlike Gecko) Starboard/3, Unknown;Unknown;Unknown;0/Unknown (Chromium, Starboard, Wired)',
    ...
    async getAuthHeaders() {
        if (this.oauth) {
            ...
            if (accessToken) {
                logger('debug', 'YouTube-TV_DOWN', 'Successfully acquired access token for authentication.');
                return {
                    Authorization: `Bearer ${accessToken}`
                };
            }
        }
        logger('debug', 'YouTube-TV_DOWN', 'No access token available. Proceeding without authentication.');
        return {};
    }
    /**
    ...
    async resolve(url, _type, context, cipherManager) {
        ...
        switch (urlType) {
            ...
            case YOUTUBE_CONSTANTS.SHORTS: {
                ...
                if (!videoIdMatch?.[1]) {
                    logger('error', 'YouTube-TV_DOWN', `Could not parse video ID from URL: ${url}`);
                    return {
                        loadType: 'error',
                        exception: {
            ...
            case YOUTUBE_CONSTANTS.PLAYLIST: {
                ...
                if (!playlistIdMatch?.[1]) {
                    logger('error', 'YouTube-TV_DOWN', `Could not parse playlist ID from URL: ${url}`);
                    return {
                        loadType: 'error',
                        exception: {
                ...
                if (statusCode !== 200) {
                    ...
                    logger('error', 'YouTube-TV_DOWN', `Error loading playlist ${playlistId}: ${errMsg}`);
                    return {
                        loadType: 'error',
                        exception: {
                            message: errMsg,
    ...
    async getTrackUrl(decodedTrack, context, cipherManager, itag, proxy) {
        ...
        logger('debug', 'YouTube-TV_DOWN', `Getting stream URL for: ${decodedTrack.title} (ID: ${decodedTrack.identifier}) on ${sourceName}`);
        const headers = await this.getAuthHeaders();
