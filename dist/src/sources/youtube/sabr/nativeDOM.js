...
let dynamicClassCache;
// biome-ignore lint/suspicious/noExplicitAny: event ctor protos hold arbitrary prototype shapes
let EVENT_CTOR_PROTOS;
let EVENT_TYPE_MAP;
let EVENT_TYPE_KEYS;
let ELEMENT_STRING_PROPS;
let ELEMENT_BOOL_PROPS;
let ELEMENT_ZERO_PROPS;
let SHADOW_ROOT_PROTO;
let ANIMATION_OBJ;
let CANVAS_2D_CTX;
let DEFERRED_REQUEST_OBJ;
...
function _init() {
    ...
    function getDummyConstructor(name) {
    ...
    DEFERRED_REQUEST_OBJ = Object.freeze(Object.assign(Object.create(DeferredRequest.prototype), { activated: true }));
    ...
    function getEventCtorProto(constructorName) {
        ...
        proto.pageY = 0;
        proto.detail = 0;
        proto.initEvent = function (type, bubbles, cancelable) {
            this.type = type;
            this.bubbles = bubbles;
            this.cancelable = cancelable;
        };
        proto.initMouseEvent = function (type, bubbles, cancelable, _view, detail, screenX, screenY, clientX, clientY, ctrlKey, altKey, shiftKey, metaKey, button, _relatedTarget) {
            this.type = type;
            this.bubbles = bubbles;
            this.cancelable = cancelable;
            this.detail = detail;
        ...
        proto.initUIEvent = function (type, bubbles, cancelable, _view, detail) {
            this.type = type;
            this.bubbles = bubbles;
            this.cancelable = cancelable;
            this.detail = detail;
    ...
    Object.defineProperties(Element.prototype, {
        ...
        attachShadow: {
            value(init) {
                const shadowRoot = Object.create(SHADOW_ROOT_PROTO);
    ...
    Object.defineProperties(Document.prototype, {
        ...
        createDocumentFragment: {
            value() {
                const frag = Object.create(Node.prototype);
                frag.nodeType = 11;
                return frag;
            },
            writable: true,
...
function isConstructorName(prop) {
...
export class NativeDOM {
    ...
    constructor(options) {
        ...
        const locationMock = Object.create(Location.prototype);
        ...
        const elementProxyHandler = {
            get(target, prop, receiver) {
                ...
                if (ELEMENT_BOOL_PROPS.has(prop))
                    return false;
                if (ELEMENT_ZERO_PROPS.has(prop))
                    return 0;
                if (prop === 'tabIndex')
                    return -1;
                return Reflect.get(target, prop, receiver);
        ...
        const createElement = (tagName) => {
            ...
            const styleMock = new Proxy(Object.create(CSSStyleDeclaration.prototype), cssProxyHandler);
            ...
            if (tag === 'iframe') {
                ...
                const buildIframe = () => {
                    if (_contentWindow)
                        return;
                    const subDocument = Object.create(HTMLDocument.prototype);
                    ...
                    subDocument.createEvent = (type) => documentMock.createEvent(type);
                    ...
                    const subWindow = Object.create(Window.prototype);
                ...
                element.getContext = (type) => type === '2d' ? CANVAS_2D_CTX : null;
            }
        ...
        const createEvent = (type) => {
            ...
            const event = Object.create(getEventCtorProto(constructorName));
            event.type = type;
            event.timeStamp = performance.now();
            return event;
        };
        const documentMock = Object.create(HTMLDocument.prototype);
        ...
        documentMock.getElementsByTagNameNS = (_ns, name) => documentMock.getElementsByTagName(name);
        documentMock.addEventListener = () => { };
        documentMock.removeEventListener = () => { };
        documentMock.hasFocus = () => true;
        const windowMock = Object.create(Window.prototype);
        ...
        windowMock.atob = (str) => Buffer.from(str, 'base64').toString('binary');
        windowMock.close = () => { };
        windowMock.getComputedStyle = (el) => el.style || {};
        windowMock.cancelIdleCallback = () => { };
        windowMock.requestIdleCallback = (cb) => setTimeout(cb, 1);
        windowMock.credentialless = false;
        windowMock.offscreenBuffering = true;
        windowMock.isSecureContext = true;
        windowMock.reportError = (err) => {
            console.error(err);
        };
        windowMock.history = {
            length: 1,
        ...
        const makeWindowProxy = (winTarget) => new Proxy(winTarget, {
            has(target, prop) {
                if (typeof prop !== 'string')
            ...
            get(target, prop, receiver) {
                ...
                if (Reflect.has(target, prop))
                    return Reflect.get(target, prop, receiver);
                if (isConstructorName(prop))
                    return getDummyConstructor(prop);
                if (prop.length > 2 &&
                    prop.charCodeAt(0) === 111 &&
                    prop.charCodeAt(1) === 110)
                    return null;
