...
export class DuckingController {
    ...
    updateConfig(config) {
        this.config = { ...config };
        if (!config.enabled && this.isDucked)
            this._restore(0);
    }
    getTargetVolume(defaultVolume = 1.0) {
    ...
    setStreamControl(control) {
        ...
        if (this.isDucked && this.streamControl) {
            try {
                ...
                logger('error', 'Ducking', `Failed to reapply ducked volume for guild ${this.guildId}: ${err.message}`);
            }
        }
    }
    attach(connection) {
    ...
    _registerSpeech(ssrc, rms = 0) {
        ...
        if (!wasDucked || (isSignificantChange && now - this.lastFadeTime > 100)) {
            ...
            try {
                ...
                logger('error', 'Ducking', `Failed to dynamically duck volume for guild ${this.guildId}: ${err.message}`);
            }
        }
    }
    _restore(overrideDuration) {
        if (!this.isDucked)
            return;
        this.isDucked = false;
        ...
        try {
            ...
            logger('error', 'Ducking', `Failed to restore volume for guild ${this.guildId}: ${err.message}`);
        }
    }
}
