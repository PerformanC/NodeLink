 ...
 */
export function inferGenreProfile(bpm, bands, energy, vocalActivity) {
    if (!bpm || !bands || energy === undefined)
        return 'universal';
    ...
    set(trackId, profile) {
        if (profileCacheMap.size >= MAX_CACHE_SIZE) {
            ...
            if (firstKey !== undefined)
                profileCacheMap.delete(firstKey);
        }
 ...
 */
export function detectIntroBoundary(pcm, sampleRate) {
    ...
    const windowBytes = Math.max(bytesPerSample, Math.floor(50 * bytesPerMs) - (Math.floor(50 * bytesPerMs) % bytesPerSample));
    let silenceEndMs = 0;
    ...
    const introBoundaryMs = foundFirstBeat
        ...
        : silenceEndMs;
    const structuredIntro = foundFirstBeat &&
 ...
 */
export function inspectTrackIntroQuick(pcm, sampleRate) {
 ...
 */
export function inspectTrackOutroQuick(pcm, sampleRate, trackId, windowStartMs, _trackLengthMs) {
    const bytesPerSample = CHANNELS * BYTES_PER_SAMPLE;
    const bytesPerMs = (sampleRate * bytesPerSample) / 1000;
    ...
    const scanWindowBytes = Math.max(bytesPerSample, Math.floor(50 * bytesPerMs) - (Math.floor(50 * bytesPerMs) % bytesPerSample));
    const totalMs = pcm.length / bytesPerMs;
    const energySamples = [];
    ...
    let energyProfileType;
    if (lastThirdAvg < avgEnergy * 0.1)
        energyProfileType = 'cliff';
    else if (lastThirdAvg < avgEnergy * 0.4)
        energyProfileType = 'fade';
    else if (lastThirdAvg < avgEnergy * 0.7)
        energyProfileType = 'decay';
    else
        energyProfileType = 'sustained';
    const outroProfile = {
        ...
        energyDecayStartMs: Math.round(energySamples[decayStartIdx]?.ms ?? windowStartMs),
 ...
 */
export function getHarmonicDistance(keyA, keyB, confA = 0.5, confB = 0.5) {
    if (!keyA || !keyB) {
        return {
            distance: -1,
            compatible: true,
 ...
 */
export function matchTempo(mainBpm, nextBpm) {
    if (!mainBpm || !nextBpm || mainBpm <= 0 || nextBpm <= 0)
        return null;
    let aligned = nextBpm;
 ...
 */
export function evaluatePlacementAnalysis(outgoing, incoming, _requestedDurationMs = 6000, availableMs) {
    const harmonic = getHarmonicDistance(outgoing.key, incoming.key, outgoing.keyConfidence, incoming.keyConfidence);
    ...
    const outroDecayStartMs = outroProfile?.energyDecayStartMs ?? Math.max(0, outgoingDurationMs - 12000);
    const naturalOutroPointMs = outroProfile?.naturalOutroPointMs ?? Math.max(0, outgoingDurationMs - 6000);
    ...
    const isOutroDecaying = outroProfile?.energyProfile === 'decay' ||
        outroProfile?.energyProfile === 'fade' ||
        outgoing.energy <= 0.05;
    const isExactOrRelativeHarmonic = harmonic.relation === 'exact-key-match' ||
    ...
    const isShortOutro = outgoingDurationMs - naturalOutroPointMs < 3500 &&
        outgoingDurationMs > 15000;
    const _effectiveAvailableMs = isShortOutro
        ? Math.max(availableMs ?? 6000, 6000)
        : availableMs;
    const harmonicEvidenceMultiplier = harmonic.confidence >= 0.2 ? 1.0 : Math.max(0.5, harmonic.confidence / 0.2);
    ...
    const vocalClash = outgoing.vocalActivity >= 0.35 && incoming.vocalActivity >= 0.35;
    const vocalScore = vocalClash ? 0.35 : 0.9;
    const outroIntroContinuityScore = silenceEndMs < 200 && isOutroDecaying
        ? 0.95
        : silenceEndMs < 500
            ? 0.8
            : 0.5;
    const candidate1Overlap = Math.min(availableMs ?? 8000, Math.max(3500, Math.min(8000, Math.round(beatMs * 8))));
    const candidate1StructuralScore = isExactOrRelativeHarmonic && (isOutroDecaying || silenceEndMs < 200)
        ? 0.95
        : 0.7;
    const candidate1Total = candidate1StructuralScore * 0.3 +
        rhythmicScore * 0.3 +
    ...
    const candidate2Overlap = Math.min(availableMs ?? 12000, Math.max(3500, Math.round(beatMs * 8)));
    const candidate2Archetype = tempo?.compatible && outgoing.bands.low > 0.02 && incoming.bands.low > 0.02
        ? 'bass-swap'
        : 'phrase-transition';
    const candidate2LowEndScore = outgoing.bands.low > 0.02 && incoming.bands.low > 0.02 ? 0.9 : 0.6;
    const candidate2StructuralScore = tempo?.compatible ? 0.85 : 0.6;
    const candidate2Total = rhythmicScore * 0.35 +
    ...
    const candidate2 = {
        ...
        outgoingTargetMs: Math.max(0, outgoingDurationMs - candidate2Overlap - 2000),
    ...
    const candidate3Overlap = Math.min(availableMs ?? 6000, Math.max(2000, Math.round(outgoingDurationMs - naturalOutroPointMs + 2000)));
    ...
    const candidate3Total = candidate3DecayScore * 0.4 +
    ...
    const candidate4Total = harmonicScore * 0.3 +
        rhythmicScore * 0.3 +
        0.6 * 0.2 +
    ...
    const isOutgoingCutReady = outgoing.energy <= 0.05 ||
        (outroProfile && outroProfile.energyProfile === 'fade');
    const candidate5StructuralScore = (isIncomingImpact || isOutgoingCutReady) && !isExactOrRelativeHarmonic
        ? 0.9
        : isIncomingImpact
            ? 0.75
            : 0.4;
    const candidate5EnergyScore = isIncomingImpact ? 0.9 : 0.5;
    const candidate5Total = candidate5StructuralScore * 0.4 +
        rhythmicScore * 0.3 +
    ...
    const candidate6Overlap = Math.min(availableMs ?? 6000, Math.max(3500, Math.min(6000, Math.round(vocalEntryMs > 0 ? vocalEntryMs + 1800 : beatMs * 8))));
    const candidate6VocalScore = hasIncomingVocalRunway && outgoing.vocalActivity > 0 ? 0.95 : 0.5;
    const candidate6StructuralScore = hasIncomingVocalRunway ? 0.88 : 0.55;
    const candidate6Total = candidate6VocalScore * 0.4 +
    ...
    const candidate6 = {
        ...
        outgoingTargetMs: lastPhraseEndMs > 0 ? lastPhraseEndMs : naturalOutroPointMs,
        incomingStartMs: silenceEndMs,
    ...
    const candidate7DecayScore = isOutroDecaying && isAcousticLowEnergy ? 0.95 : 0.4;
    const candidate7BreathScore = isAcousticLowEnergy ? 0.9 : 0.4;
    const candidate7Total = candidate7DecayScore * 0.4 +
    ...
    const candidate8Overlap = Math.min(availableMs ?? 16000, Math.max(4000, Math.min(16000, Math.round(beatMs * 16))));
    const candidate8RhythmicScore = isBpmTightMatch
        ? 0.95
        : tempo
            ? tempo.score
            : 0.45;
    const candidate8StructuralScore = isBpmTightMatch && isBothRhythmic ? 0.9 : 0.5;
    const candidate8Total = candidate8RhythmicScore * 0.4 +
    ...
    const isIncomingHighEnergy = incoming.energy >= outgoing.energy * 1.25 || incoming.energy >= 0.35;
    const candidate9Overlap = Math.min(availableMs ?? 8000, Math.max(3000, Math.round(beatMs * 8)));
    const candidate9EnergyScore = isIncomingHighEnergy ? 0.92 : 0.5;
    const candidate9StructuralScore = isIncomingHighEnergy && firstBeatMs <= 300 ? 0.88 : 0.55;
    const candidate9Total = candidate9StructuralScore * 0.35 +
    ...
    const isTapeStopFeasible = (isTempoClash || !isExactOrRelativeHarmonic) &&
        outgoing.energy >= 0.15 &&
        firstBeatMs <= 300;
    const candidate10Overlap = Math.min(availableMs ?? 1500, 1200);
    const candidate10Total = (isTapeStopFeasible ? 0.9 : 0.4) * 0.35 +
    ...
    const candidate11Overlap = Math.min(availableMs ?? 5000, Math.max(2500, Math.round(beatMs * 8)));
    const candidate11Decay = outgoing.energy >= 0.2 && isOutroDecaying;
    const candidate11Total = (candidate11Decay ? 0.92 : 0.45) * 0.35 +
        0.75 * 0.3 +
        rhythmicScore * 0.2 +
        harmonicScore * 0.15;
    const candidate11 = {
 ...
 */
export function evaluateMusicalRelationship(outgoing, incoming, requestedDurationMs = 6000, availableMs) {
    const harmonic = getHarmonicDistance(outgoing.key, incoming.key, outgoing.keyConfidence, incoming.keyConfidence);
    const tempo = matchTempo(outgoing.bpm, incoming.bpm);
    ...
    const outroProfile = 'outroProfile' in outgoing ? outgoing.outroProfile : null;
    const introProfile = 'introProfile' in incoming ? incoming.introProfile : null;
    const outgoingTransitionConf = 'transitionConfidence' in outgoing
        ? outgoing.transitionConfidence
        : (outroProfile?.analysisConfidence ?? outgoing.confidence);
    const placement = evaluatePlacementAnalysis(outgoing, incoming, requestedDurationMs, availableMs);
    ...
    const decisionReliability = harmonic.confidence * 0.3 +
    ...
    const confidenceLevel = decisionReliability >= 0.4
        ? 'strong'
        : decisionReliability >= 0.22
            ? 'likely'
            : 'uncertain';
    const effects = {
        bassSwap: archetype === 'bass-swap' ||
            archetype === 'beatmatch-blend' ||
            ((archetype === 'continuation-handoff' ||
                archetype === 'vocal-handoff') &&
                incoming.bands.low > 0.035),
        sidechain: (archetype === 'bass-swap' ||
            archetype === 'beatmatch-blend' ||
            archetype === 'continuation-handoff' ||
            archetype === 'vocal-handoff') &&
            incoming.bands.low > 0.035,
        echoTail: archetype === 'filter-sweep-dip' ||
            archetype === 'natural-decay' ||
            archetype === 'silence-breath' ||
            archetype === 'washout-delay' ||
            (archetype === 'hard-cut' && isOutroDecaying),
        spectralTilt: incoming.brightness > outgoing.brightness * 1.35 ||
            archetype === 'vocal-handoff' ||
            archetype === 'beatmatch-blend',
        ...
        stereoMorph: archetype === 'continuation-handoff' ||
            archetype === 'vocal-handoff' ||
            archetype === 'instrumental-blend' ||
            archetype === 'beatmatch-blend',
        energyLift: incoming.energy >= outgoing.energy * 1.35 || archetype === 'energy-lift',
        energyDrop: incoming.energy <= outgoing.energy * 0.65,
        multiBand: archetype === 'beatmatch-blend' ||
            archetype === 'instrumental-blend' ||
            (archetype === 'bass-swap' &&
    ...
    return {
        ...
        confidenceLevel,
        decisionReason,
        musicalCompatibilityScore: Math.round(musicalCompatibilityScore * 1000) / 1000,
