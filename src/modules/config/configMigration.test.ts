...
import { migrateConfig, toTsLiteral } from './configMigration.ts'
...
test('migrateConfig fills required compatibility defaults', () => {
  ...
  const migrated = migrateConfig(
    minimalLegacy as Record<string, unknown>
  ) as unknown as NodelinkConfig
...
test('migrateConfig keeps vanilla default config valid', () => {
  ...
  assert.equal(
    migrated.playback.playerUpdateInterval,
    defaultConfig.playback.playerUpdateInterval
  )
  assert.equal(
    migrated.api.enableTrackStreamEndpoint,
    defaultConfig.api.enableTrackStreamEndpoint
  )
...
test('toTsLiteral generates idiomatic TypeScript formatting', () => {
  ...
  assert.equal(toTsLiteral(input as Record<string, unknown>), expected)
})
