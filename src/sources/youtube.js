...
async function init() {
  playerInfo.innertubeInterval = setIntervalNow(async () => {
    debugLog("innertube", 5, {
      type: 1,
      message: "Fetching deciphering functions...",
    });

    const { body: data } = await makeRequest("https://www.youtube.com/embed", {
      method: "GET",
    }).catch((err) => {
      debugLog("innertube", 5, {
        type: 2,
        message: `Failed to access YouTube website: ${err.message}`,
      });
    });

    const { body: player } = await makeRequest(
      `https://www.youtube.com${/(?<=jsUrl":")[^"]+/.exec(data)[0]}`,
      { method: "GET" }
    ).catch((err) => {
      debugLog("innertube", 5, {
        type: 2,
        message: `Failed to fetch player.js: ${err.message}`,
      });
    });

    playerInfo.signatureTimestamp = /(?<=signatureTimestamp:)[0-9]+/.exec(
      player
    )[0];
    ...
    const sigFunction = player.match(
      new RegExp(
    ...
    const sigWrapper = player.match(
      new RegExp(`var ${functionName}={(.*?)};`, "s")
    )[1];
    ...
    if (functionName && functionName.includes("["))
      functionName = player.match(
        new RegExp(`${functionName.match(/([^[]*)\[/)[1]}=\\[(.*?)]`)
      )[1];

    const ncodeFunction = player.match(
      new RegExp(`${functionName}=function(.*?)};`, "s")
    )[1];
    playerInfo.functions[1] = `const ${functionName} = function${ncodeFunction}};${functionName}(ncode)`;

    debugLog("innertube", 5, {
      type: 1,
      message: "Successfully fetched deciphering functions.",
    });
  }, 3600000);
}
...
async function search(query, type, shouldLog) {
  return new Promise(async (resolve) => {
    if (playerInfo.functions.length !== 2)
      while (1) {
    ...
    if (shouldLog)
      debugLog("search", 4, { type: 1, sourceName: "YouTube", query });

    const { body: search } = await makeRequest(
      `https://${
        type === "ytmusic" ? "music" : "www"
    ...
    const videos =
      search.contents.sectionListRenderer.contents[
        search.contents.sectionListRenderer.contents.length - 1
    ...
    videos.forEach((video, index) => {
      ...
      if (video) {
        const track = {
          ...
          length: video.lengthText
            ? (parseInt(video.lengthText.runs[0].text.split(":")[0]) * 60 +
                parseInt(video.lengthText.runs[0].text.split(":")[1])) *
              1000
            : 0,
      ...
      if (
        ...
        tracks.length === config.options.maxResultsLength - 1
      ) {
        if (tracks.length === 0) {
          debugLog("search", 4, {
            type: 3,
  ...
  return new Promise(async (resolve) => {
    ...
    switch (checkURLType(query, type)) {
      case 2: {
        ...
        const { body: video } = await makeRequest(
          `https://${
            type === "ytmusic" ? "music" : "www"
          }.youtube.com/youtubei/v1/player?key=AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&prettyPrint=false`,
          {
            body: {
              context: playerInfo.innertube,
              videoId: identifier,
        ...
        if (video.playabilityStatus.status !== "OK") {
          debugLog("loadtracks", 4, {
            ...
            query,
            message:
              video.playabilityStatus.reason ||
              video.playabilityStatus.messages[0],
          });
      ...
      case 3: {
        ...
        const { body: playlist } = await makeRequest(
          `https://${
            type === "ytmusic" ? "music" : "www"
          }.youtube.com/youtubei/v1/next?key=AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&prettyPrint=false`,
          {
            method: "POST",
            body: {
              context: playerInfo.innertube,
        ...
        let playlistContent =
          playlist.contents.singleColumnWatchNextResults.playlist.playlist
            .contents;

        if (playlistContent.length > config.options.maxAlbumPlaylistLength)
          playlistContent = playlistContent.slice(
            0,
            config.options.maxAlbumPlaylistLength
          );
        ...
        playlistContent.forEach((video) => {
          ...
          if (video) {
            const track = {
              identifier: video.videoId,
              isSeekable: true,
              author: video.shortBylineText.runs
        ...
        return resolve({
          ...
          data: {
            info: {
              name: playlist.contents.singleColumnWatchNextResults.playlist
                .playlist.title,
              selectedTrack: 0,
            },
            pluginInfo: {},
      ...
      case 4: {
        ...
        const { body: short } = await makeRequest(
        ...
        if (short.playabilityStatus.status !== "OK") {
          debugLog("loadtracks", 4, {
            ...
            query,
            message:
              short.playabilityStatus.reason ||
              short.playabilityStatus.messages[0],
          });
...
async function retrieveStream(identifier, type, title) {
  return new Promise(async (resolve) => {
    ...
    const { body: videos } = await makeRequest(
      `https://${
        type === "ytmusic" ? "music" : "www"
      }.youtube.com/youtubei/v1/player?key=AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&prettyPrint=false`,
      {
        ...
        headers: {
          "User-Agent":
            "com.google.android.youtube/17.29.34 (Linux; U; Android 13) gzip",
        },
        method: "POST",
      }
    );
    ...
    const audio =
      videos.streamingData.adaptiveFormats.find(
        (format) => format.itag === itag
      ) ||
      videos.streamingData.adaptiveFormats.find((format) =>
        format.mimeType.startsWith("audio/")
      );

    const args = new URLSearchParams(
      audio.url || audio.signatureCipher || audio.cipher
    );
    ...
    if (audio.signatureCipher || audio.cipher)
      url += `&${args.get("sp")}=${eval(
        `const sig = ${args.get("s")};` + playerInfo.functions[0]
      )}`;

    if (args.get("n"))
      url += `&${args.get("n")}=${eval(
        `const ncode = ${args.get("s")};` + playerInfo.functions[1]
      )}`;
...
async function loadCaptions(decodedTrack, language) {
  return new Promise(async (resolve) => {
    ...
    const { body: video } = await makeRequest(
      `https://${
        decodedTrack.sourceName === "ytmusic" ? "music" : "www"
      }.youtube.com/youtubei/v1/player?key=AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&prettyPrint=false`,
      {
        body: {
          context: playerInfo.innertube,
    ...
    if (video.playabilityStatus.status !== "OK") {
      debugLog("loadcaptions", 4, {
        type: 2,
        sourceName: "YouTube",
        track: { title: decodedTrack.title, author: decodedTrack.author },
    ...
    if (language) {
      const caption =
        video.captions.playerCaptionsTracklistRenderer.captionTracks.find(
          (caption) => {
      ...
      const { body: captionsData } = await makeRequest(
        caption.baseUrl.replace("&fmt=srv3", "&fmt=json3"),
        { method: "GET" }
      ).catch((err) => {
        debugLog("loadcaptions", 4, {
          type: 2,
        ...
        return resolve({
          loadType: "error",
          data: { message: err.message, severity: "common", cause: "Unknown" },
        });
      });
      ...
      if (!video.captions) return resolve({ loadType: "empty", data: {} });

      video.captions.playerCaptionsTracklistRenderer.captionTracks.forEach(
        async (caption) => {
          const { body: captionData } = await makeRequest(
            caption.baseUrl.replace("&fmt=srv3", "&fmt=json3"),
            { method: "GET" }
          ).catch((err) => {
            debugLog("loadcaptions", 4, {
          ...
          if (
            i ===
            video.captions.playerCaptionsTracklistRenderer.captionTracks
              .length -
              1
          ) {
