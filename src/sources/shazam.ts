...
const SHAZAM_ANDROID_USER_AGENT =
  'Dalvik/2.1.0 (Linux; U; Android 15; SM-S9210 Build/af013b0.3) Shazam/v16.56.0'
...
export default class ShazamSource {
  ...
  public async resolve(url: string): Promise<SourceResult> {
    try {
      ...
      if (statusCode === 403) {
        return (
          (await this.resolveViaCommerce(url)) ?? {
            loadType: 'empty',
            data: {}
          }
        )
  ...
  private async resolveViaCommerce(url: string): Promise<SourceResult | null> {
    try {
      ...
      const catalogRes = await http1makeRequest(
        `https://api.music.apple.com/v1/catalog/US/songs/${encodeURIComponent(identifier)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Origin: 'https://www.shazam.com',
