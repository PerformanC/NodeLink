...
/**
 * Minimal LRCLIB search item used by this provider.
 */
interface LRCLibSearchItem {
  ...
  /**
   * Primary artist returned by LRCLIB.
   */
  artistName: string

  /**
   * Whether LRCLIB marked the entry as instrumental.
   */
  instrumental: boolean

  /**
   * Time-synced lyrics in LRC format when available.
   */
  syncedLyrics: string | null

  /**
   * Plain unsynced lyrics when available.
   */
  plainLyrics: string | null
}

/**
 * Track title split into artist and title components when possible.
 */
interface ParsedTrackQuery {
  /**
   * Artist extracted from the combined title, or `null` when unavailable.
   */
  artist: string | null

  /**
   * Sanitized track title fragment.
   */
  title: string
}

/**
 * Minimal runtime shape accepted by the LRCLIB provider.
 */
type LRCLibNodeLink = object
 ...
 */
function cleanMetadata(text: string, removeFeaturing = false): string {
  let result = text
 ...
 */
function parseTrackQuery(query: string): ParsedTrackQuery {
...
function parseSearchResults(
  ...
  const results: LRCLibSearchItem[] = []
...
export default class LRCLIBLyrics {
   ...
   */
  public readonly nodelink: LRCLibNodeLink
   ...
   */
  public constructor(nodelink: LRCLibNodeLink) {
   ...
   */
  private parsePlainLyrics(lyrics: string): LyricsLine[] {
    return lyrics
      .split('\n')
      .map((line) => line.trim())
   ...
   */
  private applyLineDurations(lines: LyricsLine[]): LyricsLine[] {
    ...
    return sortedLines.map((line, index) => {
      ...
      return {
        ...line,
        duration:
          nextLine && nextLine.time > line.time ? nextLine.time - line.time : 0
      }
    })
  }
   ...
   */
  private buildLyricsResult(match: LRCLibSearchItem): LyricsResult {
  ...
  public async getLyrics(trackInfo: TrackInfo): Promise<LyricsResult> {
    ...
    try {
      ...
      const results = parseSearchResults(
        response.body as string | JsonValue | JsonValue[] | null | undefined
      )
      ...
      logger(
        'error',
        'Lyrics',
        `Failed to fetch lyrics from LRCLIB: ${message}`
      )
