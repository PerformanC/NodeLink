...
/**
 * Request payload accepted by the session patch endpoint.
 */
interface SessionPatchPayload {
  /**
   * Whether the session should remain resumable.
   */
  resuming?: boolean
...
/**
 * Response payload returned by the session patch endpoint.
 */
interface SessionPatchResponse {
  /**
   * Updated resumable flag.
   */
  resuming: boolean

  /**
   * Updated resume timeout in seconds.
   */
  timeout: number
}
 ...
 */
interface SessionRouteRuntime extends ApiNodelinkServer {
  /**
...
): SessionRouteRuntime | null {
  const runtime = nodelink as ApiNodelinkServer & Partial<SessionRouteRuntime>

  if (!runtime.sessions || typeof runtime.sessions.get !== 'function') {
    return null
  }

  return runtime as SessionRouteRuntime
}
 ...
 */
function getSessionIdFromPath(parsedUrl: URL): string | null {
 ...
 */
function getSessionPatchPayload(
  body: ApiRequest['body']
): SessionPatchPayload | null {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    return null
  }

  const payload = body as SessionPatchPayload
...
async function handler(
  ...
  if (!runtime) {
    sendErrorResponse(
      ...
      'Session runtime contract is incomplete.',
      parsedUrl.pathname,
      true
    )
    return
 ...
 */
const sessionRoute: ApiRouteModule = {
  handler,
  methods: ['PATCH']
}
