...
/**
 * Runtime contract required by the version endpoint.
 *
 * The shared API server type intentionally exposes only the fields required by
 * the router itself, while this route also depends on the public `version`
 * field exposed by the main NodeLink runtime.
 */
interface VersionedApiNodelinkServer extends ApiNodelinkServer {
  /**
   * Semantic version string exposed by the running NodeLink instance.
   */
  version: string
}
 ...
 */
function hasVersionField(
  nodelink: ApiNodelinkServer
): nodelink is VersionedApiNodelinkServer {
 ...
 * be declared here.
 */
const versionRoute: ApiRouteModule = {
  handler
}
