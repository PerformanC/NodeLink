...
interface ProfilerEndpointConfig {
  /**
   * Whether profiler patch endpoints are enabled.
   */
  patchEnabled: boolean

  /**
   * Whether external clients may access the profiler endpoints.
   */
...
/**
 * Single line returned in the file snippet payload.
 */
interface FileSnippetLine {
  /**
  ...
  /**
   * Raw source text for the line.
   */
  text: string
}

/**
 * Minimal request body accepted by the profiler file endpoint.
 */
interface ProfilerFileBodyInput {
  /**
   * Access code optionally supplied in the request body.
   */
  code?: string
}

/**
 * Serialized response returned by the profiler file route.
 */
interface ProfilerFileResponse {
  /**
   * Final path used to read the file.
   */
  path: string

  /**
   * Requested focal line.
   */
  line: number

  /**
   * First line included in the snippet.
   */
  start: number

  /**
   * Last line included in the snippet.
   */
  end: number

  /**
   * Total amount of lines in the resolved file.
   */
  totalLines: number

  /**
   * Snippet lines around the requested focus line.
   */
  snippet: FileSnippetLine[]
}
 ...
 */
function parsePositiveLine(value: string | null, fallback = 1): number {
 ...
 */
function getSuppliedCode(req: ApiRequest, parsedUrl: URL): string | null {
  ...
  if (isObjectRecord(req.body)) {
    const bodyCode = (req.body as ProfilerFileBodyInput).code
 ...
 */
function getPathCandidates(absolutePath: string): string[] {
 ...
 */
async function resolveReadablePath(candidates: string[]): Promise<string> {
 ...
 */
async function buildSnippetResponse(
  resolvedPath: string,
  line: number,
  context: number
): Promise<ProfilerFileResponse> {
  const content = await fsPromises.readFile(resolvedPath, 'utf8')
  const lines = content.split('\n')
  const start = Math.max(1, line - context)
  const end = Math.min(lines.length, line + context)
  const snippet: FileSnippetLine[] = []
...
async function handler(
  ...
  const context = Math.min(
    60,
    Math.max(3, parsePositiveLine(parsedUrl.searchParams.get('context'), 8))
  )
...
const profilerFileRoute: ApiRouteModule = {
  handler,
  methods: ['GET']
}
