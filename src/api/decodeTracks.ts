 ...
 */
const INVALID_ENCODED_TRACKS_MESSAGE =
 ...
 */
function isEncodedTrackList(body: ApiRequest['body']): body is string[] {
  return (
    Array.isArray(body) &&
    body.length > 0 &&
    body.every(
      (encodedTrack) =>
        typeof encodedTrack === 'string' && encodedTrack.trim().length > 0
    )
  )
}
 ...
 */
function normalizeEncodedTracks(encodedTracks: string[]): string[] {
 ...
 */
function decodeTrackBatch(encodedTracks: string[]): EncodedTrackPayload[] {
  const decodedTracks: EncodedTrackPayload[] = []
...
function handler(
  ...
  if (!isEncodedTrackList(req.body)) {
    sendErrorResponse(
      ...
      400,
      'Invalid request',
      INVALID_ENCODED_TRACKS_MESSAGE,
      parsedUrl.pathname,
      true
    )
    return
  ...
  try {
    ...
    logger(
      'error',
      'Tracks',
      `Failed to decode one or more tracks from a batch of ${encodedTracks.length}.`,
      error
    )
    sendErrorResponse(
      req,
 ...
 */
const decodeTracksRoute: ApiRouteModule = {
  handler,
  methods: ['POST']
}
