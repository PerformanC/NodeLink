...
import { MINIMUM_NODE_VERSION } from '../constants.ts'
...
function _parseVersionParts(version: string): number[] {
  return version
    .replace(/^v/, '')
    .split('.')
...
function _isVersionAtLeast(current: string, minimum: string): boolean {
...
function memoryTrace(stage: string): void {
...
async function validateRuntime(
  ...
  if (!isSupported) {
    throw new Error(
      `Unsupported Node.js runtime (${process.version}). Baseline is v${MINIMUM_NODE_VERSION}. Please update to Node.js LTS.`
    )
  }

  if (!isLts) {
    const isAboveOrAtLts = Boolean(
      latestLts && _isVersionAtLeast(process.version, latestLts)
    )
    ...
    logger(
      'info',
      'Server',
      `Runtime ${process.version} is supported, but below latest LTS (${latestLts}). Consider updating.`
    )
  }
}
