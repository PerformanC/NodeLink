...
export class OpusHead {
  constructor(data) {
    ...
    this.preskip =
      data.preskip !== null ? data.preskip : data.sampleRate * (80 / 1000);
...
function serialiseHeaderTypeFlag(flags) {
  return (
    (flags.continuedPacket ? 0x01 : 0) +
    (flags.firstPage ? 0x02 : 0) +
    (flags.lastPage ? 0x04 : 0)
  );
}
...
export class OggLogicalBitstream extends Transform {
  constructor(options) {
    ...
    if (Reflect.has(this.options.pageSizeControl, "maxSegments")) {
      ...
      this.pageSizeController = (_packet, lacingValues) =>
    ...
    this.opusTags =
      options.opusTags !== null ? options.opusTags : new OpusTags();
    this.writeHeaderPages([
      [options.opusHead.toBuffer()],
      [this.opusTags.toBuffer()],
    ]);
  }
  calculateGranulePosition(packets) {
    const sampleRate = this.opusHead.sampleRate / 1000;
    const newCount = packets.reduce(
      (acc, val) => acc + sampleRate * FRAME_SIZE_MAP[val[0] >> 3],
      0
    );
  ...
  writePacket(packet) {
    ...
    if (
      this.pageSizeController(packet, lacingValues) ||
      lacingValues.length + this.lacingValues.length > 255
    )
      this.writePage();
