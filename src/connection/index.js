...
if (
  typeof config.options.threshold !== "boolean" &&
  typeof config.options.threshold !== "number"
)
  throw new Error("Threshold must be a boolean or a number.");

if (
  typeof config.options.playerUpdateInterval !== "boolean" &&
  typeof config.options.playerUpdateInterval !== "number"
)
  throw new Error("Player update interval must be a boolean or a number.");

if (
  typeof config.options.statsInterval !== "boolean" &&
  typeof config.options.statsInterval !== "number"
)
...
if (
  typeof config.options.autoUpdate[2] !== "boolean" &&
  typeof config.options.autoUpdate[2] !== "number"
)
...
if (
  !["bandcamp", "deezer", "soundcloud", "youtube", "ytmusic"].includes(
    config.search.defaultSearchSource
  )
)
  throw new Error(
    'Default search source must be either "bandcamp", "deezer", "soundcloud", "youtube" or "ytmusic".'
  );
...
if (
  config.search.sources.spotify.enabled &&
  !config.search.sources.spotify.market
)
  throw new Error("Spotify is enabled but no market was provided.");

if (
  config.search.sources.deezer.enabled &&
  (!config.search.sources.deezer.decryptionKey ||
    !config.search.sources.deezer.apiKey)
)
  throw new Error(
    "Deezer is enabled but no decryption key or API key was provided."
  );

if (
  config.search.sources.soundcloud.enabled &&
  !config.search.sources.soundcloud.clientId
)
  throw new Error("SoundCloud is enabled but no client ID was provided.");

if (config.options.autoUpdate[2])
  setInterval(() => {
    checkForUpdates();
...
v4.on("/v4/websocket", (ws, req) => {
  if (req.headers.authorization !== config.server.password) {
    console.log(
      "[\u001b[31mwebsocket\u001b[39m]: Invalid password. Closing connection..."
    );
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (pathname === "/v4/websocket")
    v4.handleUpgrade(req, socket, head, { isNodeLink: true }, (ws) =>
      v4.emit("/v4/websocket", ws, req)
    );

  if (pathname === "/connection/data")
    v4.handleUpgrade(req, socket, head, {}, (ws) =>
      v4.emit("/connection/data", ws, req)
    );
});

v4.on("error", (err) => {
  console.error(
    `[\u001b[31mwebsocket\u001b[37m]: Error: \u001b[31m${err}\u001b[37m`
  );
});

server.on("error", (err) => {
  console.error(
    `[\u001b[31mhttp\u001b[37m]: Error: \u001b[31m${err}\u001b[37m`
  );
});

server.listen(config.server.port || 2333, () => {
  console.log(
    `[\u001b[32mwebsocket\u001b[37m]: Listening on port \u001b[94m${
