...
class VoiceConnection {
  constructor(guildId, client) {
    //this.connection - Not set to anything, but used in the constructor.
    this.client = client;
    this.cache = {
      startedAt: 0,
      pauseTime: [0, 0],
    ...
    //this.stateInterval - Not set to anything, but used in the constructor.
  ...
  setup() {
    ...
    this.connection.on("speakStart", (userId, ssrc) =>
      inputHandler.handleStartSpeaking(ssrc, userId, this.config.guildId)
    );
        ...
        case "disconnected": {
          ...
          try {
            await waitForEvent(
              this.connection,
              "stateChange",
              (_oldState, newState) => newState.status === "ready",
              config.options.threshold || undefined
            );
          } catch (e) {
            debugLog("websocketClosed", 2, {
              track: this.config.track?.info,
              exception: constants.VoiceWSCloseCodes[newState.closeCode],
            });
  ...
  async getResource(decodedTrack, urlInfo) {
    return new Promise(async (resolve) => {
      const streamInfo = await sources.getTrackStream(
        decodedTrack,
        urlInfo.url,
        urlInfo.protocol,
        urlInfo.additionalData
      );
  ...
  async play(track, decodedTrack, noReplace) {
    ...
    if (Object.keys(this.config.filters).length > 0) {
      ...
      //filterEnabled = true
      resource = await filter.getResource(
        this.config.guildId,
        decodedTrack,
        urlInfo.protocol,
        urlInfo.url,
    ...
    if (resource.exception) {
      ...
      debugLog("trackException", 2, {
        track: decodedTrack,
        exception: resource.exception.message,
      });
    ...
    try {
      await waitForEvent(
        this.connection,
        "playerStateChange",
        (_oldState, newState) => newState.status === "playing",
        config.options.threshold || undefined
      );
  ...
  async filters(filters) {
    ...
    const resource = await filter.getResource(
      this.config.guildId,
      this.config.track.info,
      protocol,
      this.cache.url,
      this._getRealTime(),
      filters.endTime,
